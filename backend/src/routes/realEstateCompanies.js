...
router.post('/:id/invite/:invitationId/accept', async (req, res) => {
  try {
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:member_joined:${user._id}`,
      route: `/real-estate-companies/${company._id}`,
      tab: 'members',
      entityType: 'company',
      entityId: company._id,
...
router.delete('/:id/members/:userId', async (req, res) => {
  try {
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:removed:${req.params.userId}`,
...
router.post('/:id/apply', async (req, res) => {
  try {
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:application:${user._id}:${company.founderId}`,
      route: appRoute,
      tab: 'members',
      entityType: 'company',
      entityId: company._id,
      ...
      if (m.userId?.toString() !== company.founderId?.toString() && m.userId?.toString() !== req.user._id.toString()) {
        if (['ceo', 'director', 'officer'].includes(m.role)) {
          await enqueueNotification({
            ...
            title: 'New Application',
            message: `${user.username} applied to join "${company.name}"`,
            eventKey: `company:${company._id}:application:${user._id}:${m.userId}`,
            route: appRoute,
            tab: 'members',
            entityType: 'company',
            entityId: company._id,
...
router.post('/:id/applications/:appId/approve', async (req, res) => {
  try {
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:application:${applicant._id}:approved`,
      route: `/real-estate-companies/${company._id}`,
      tab: 'members',
      entityType: 'company',
      entityId: company._id,
...
router.post('/:id/loan-requests', async (req, res) => {
  try {
    ...
    for (const m of company.members) {
      if (m.userId?.toString() !== req.user._id.toString()) {
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:loan:${loanRequest._id}:vote_request:${m.userId}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'loans',
          entityType: 'company',
          entityId: company._id,
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:loan:${loanRequest._id}:submitted`,
      route: `/real-estate-companies/${company._id}`,
      tab: 'loans',
      entityType: 'company',
      entityId: company._id,
...
router.post('/:id/property-purchase-requests', async (req, res) => {
  try {
    ...
    for (const m of company.members) {
      if (m.userId?.toString() !== req.user._id.toString()) {
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:purchase:${purchaseRequest._id}:vote_request:${m.userId}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'properties',
          entityType: 'company',
          entityId: company._id,
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:purchase:${purchaseRequest._id}:submitted`,
      route: `/real-estate-companies/${company._id}`,
      tab: 'properties',
      entityType: 'company',
      entityId: company._id,
...
router.post('/:id/development-requests', async (req, res) => {
  try {
    ...
    for (const m of company.members) {
      if (m.userId?.toString() !== req.user._id.toString()) {
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:dev:${devRequest._id}:vote_request:${m.userId}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'properties',
          entityType: 'company',
          entityId: company._id,
    ...
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:dev:${devRequest._id}:submitted`,
      route: `/real-estate-companies/${company._id}`,
      tab: 'properties',
      entityType: 'company',
      entityId: company._id,
      ...
      try {
        ...
        for (const m of company.members) {
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:dev:${devReq._id}:executed:${m.userId}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'properties',
            entityType: 'company',
            entityId: company._id,
