  ...
  while (getLevelFromTotalXP(company.xp) > company.level) {
    ...
    for (const userId of memberUserIds) {
      await enqueueNotification({
        ...
        eventKey: `company:${company._id}:level_up:${company.level}:${userId}`,
    ...
    for (const milestone of newMilestones) {
      ...
      for (const userId of memberUserIds) {
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:milestone:${milestone.id}:${userId}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'overview',
          entityType: 'company',
          entityId: company._id,
    ...
    for (const lr of company.loanRequests) {
      ...
      if (!founderVoted && !founderIsProposer && ageTicks >= FOUNDER_AUTO_VOTE_DELAY_TICKS) {
        ...
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:loan:${lr._id}:auto_vote:${lr.requestedBy}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'loans',
          entityType: 'company',
          entityId: company._id,
      ...
      if (ageTicks >= LOAN_REQUEST_EXPIRE_TICKS && lr.status === 'pending') {
        ...
        if (totalVoters > 0 && yesVotes / totalVoters >= LOAN_REQUEST_VOTE_THRESHOLD) {
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:loan:${lr._id}:approved:${lr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'loans',
            entityType: 'company',
            entityId: company._id,
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:loan:${lr._id}:expired:${lr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'loans',
            entityType: 'company',
            entityId: company._id,
      ...
      if (lr.status === 'approved' && lr.loanId == null) {
        ...
        if (activeLoans.length >= MAX_ACTIVE_LOANS) {
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:loan:${lr._id}:exec_failed:max_loans:${lr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'loans',
            entityType: 'company',
            entityId: company._id,
        ...
        if (existingDebt + lr.principal > companyNetWorth) {
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:loan:${lr._id}:exec_failed:debt:${lr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'loans',
            entityType: 'company',
            entityId: company._id,
        ...
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:loan:${lr._id}:executed:${lr.requestedBy}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'loans',
          entityType: 'company',
          entityId: company._id,
    ...
    for (const dr of company.developmentRequests) {
      ...
      if (!ceoVoted && !ceoIsProposer && ageTicks >= DEV_REQUEST_AUTO_VOTE_DELAY_TICKS) {
        ...
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:dev:${dr._id}:auto_vote:${dr.requestedBy}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'properties',
          entityType: 'company',
          entityId: company._id,
      ...
      if (ageTicks >= DEV_REQUEST_EXPIRE_TICKS && dr.status === 'pending') {
        ...
        if (totalVoters > 0 && yesVotes / totalVoters >= LOAN_REQUEST_VOTE_THRESHOLD) {
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:dev:${dr._id}:approved:${dr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'properties',
            entityType: 'company',
            entityId: company._id,
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:dev:${dr._id}:expired:${dr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'properties',
            entityType: 'company',
            entityId: company._id,
    ...
    for (const ppr of company.propertyPurchaseRequests) {
      ...
      if (!ceoVoted && !ceoIsProposer && ageTicks >= PROPERTY_PURCHASE_AUTO_VOTE_DELAY_TICKS) {
        ...
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:purchase:${ppr._id}:auto_vote:${ppr.requestedBy}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'properties',
          entityType: 'company',
          entityId: company._id,
        ...
        if (totalVoters > 0 && yesVotes / totalVoters >= 0.5) {
          ...
          if (!property) {
            ...
            await enqueueNotification({
              ...
              eventKey: `company:${company._id}:purchase:${ppr._id}:unavailable:${ppr.requestedBy}`,
              route: `/real-estate-companies/${company._id}`,
              tab: 'properties',
              entityType: 'company',
              entityId: company._id,
          ...
          if (company.treasury.balance < property.currentPrice) {
            ...
            await enqueueNotification({
              ...
              eventKey: `company:${company._id}:purchase:${ppr._id}:insufficient_funds:${ppr.requestedBy}`,
              route: `/real-estate-companies/${company._id}`,
              tab: 'properties',
              entityType: 'company',
              entityId: company._id,
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:purchase:${ppr._id}:approved:${ppr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'properties',
            entityType: 'company',
            entityId: company._id,
          ...
          await enqueueNotification({
            ...
            eventKey: `company:${company._id}:purchase:${ppr._id}:expired:${ppr.requestedBy}`,
            route: `/real-estate-companies/${company._id}`,
            tab: 'properties',
            entityType: 'company',
            entityId: company._id,
