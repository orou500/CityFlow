    ...
    if (request && request.status === 'pending') {
      ...
      for (const member of company.members) {
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:loan:${proposalId}:expired:${member.userId}`,
    ...
    if (request && request.status === 'pending') {
      ...
      for (const member of company.members) {
        await enqueueNotification({
          ...
          eventKey: `company:${company._id}:dev:${proposalId}:expired:${member.userId}`,
          route: `/real-estate-companies/${company._id}`,
          tab: 'properties',
          entityType: 'company',
          entityId: company._id,
      ...
      if (companyInvestments) {
        for (const member of companyInvestments.members) {
          await enqueueNotification({
            ...
            eventKey: `company:${companyId}:investment:${proposalId}:expired:${member.userId}`,
    ...
    if (request && request.status === 'pending') {
      ...
      for (const member of company.members) {
        await enqueueNotification({
          ...
          eventKey: `company:${companyId}:purchase:${proposalId}:expired:${member.userId}`,
          route: `/real-estate-companies/${companyId}`,
          tab: 'properties',
          entityType: 'company',
          entityId: companyId,
...
async function handleInvestmentMaturity(data) {
  ...
  for (const member of company.members) {
    await enqueueNotification({
      ...
      eventKey: `company:${company._id}:investment:${investmentId}:matured:${member.userId}`,
      route: `/real-estate-companies/${company._id}`,
      tab: 'investments',
      entityType: 'company',
      entityId: company._id,
...
async function handleOfferExpiration(data) {
  ...
  await enqueueNotification({
    ...
    eventKey: `offer:${offerId}:expired:${offer.buyerId}`,
    route: '/marketplace',
  ...
  if (offer.sellerId) {
    await enqueueNotification({
      ...
      eventKey: `offer:${offerId}:expired:${offer.sellerId}`,
      route: '/marketplace',
      entityType: 'marketplace',
      relatedId: offer.propertyId,
      global: false,
