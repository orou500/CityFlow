...
export default function UserProfilePage() {
  ...
  const isOwnerSupporter = !!(currentUser?.supporter?.badge && currentUser.supporter.badge !== 'none');
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
          ...
          <UserIdentity
            ...
            avatarTextClass="text-3xl"
            usernameClass="text-2xl font-bold text-gray-900 dark:text-white"
            avatarOverlay={
              isOwner ? (
                <>
                  <button
          ...
          <div className="px-6 md:px-8 pb-6 md:pb-8 text-center md:text-start">
            <p className="text-sm text-gray-500 dark:text-gray-400">@{profileUser.username}</p>
            {profileUser.donationStats?.donorSince && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                {t('supporterIdentity.supporterSince')}{' '}
                {new Date(profileUser.donationStats.donorSince).toLocaleDateString()}
              </p>
            )}
            {profileUser.bio && (
            ...
            {isOwner && (
              <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-4">
                <Link
                  to={isOwnerSupporter ? '/supporter-style' : '/donate'}
                  className="shrink-0 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors"
                >
                  {isOwnerSupporter ? t('supporterIdentity.customizeProfile') : t('supporterIdentity.supportCityFlow')}
                </Link>
                <button
        ...
        {isOwner && oauthStatus?.providers && (
          <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
            <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3">{t('profile.connectedAccounts')}</h3>
