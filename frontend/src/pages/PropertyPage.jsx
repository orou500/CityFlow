...
function PriceHistoryChart({ history }) {
  ...
  const showPoint = (i) => { if (!pinned) setActiveIdx(i); };
  const hidePoint = () => { if (!pinned) setActiveIdx(null); };
  const clickPoint = (i) => {
    ...
    <div className="relative" onMouseLeave={hidePoint}>
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto overflow-visible">
        <polyline
          ...
          points={polyline}
        />
        <text x={isRtl ? w - padding : padding} y={padding + 10} className="fill-gray-600 dark:fill-gray-300 text-[10px]" textAnchor={isRtl ? 'end' : 'start'}>{fmt(min)}</text>
        <text x={isRtl ? w - padding : padding} y={h - padding - 4} className="fill-gray-600 dark:fill-gray-300 text-[10px]" textAnchor={isRtl ? 'end' : 'start'}>{fmt(max)}</text>
        <text x={isRtl ? padding : w - padding} y={h - padding - 4} className="fill-gray-600 dark:fill-gray-300 text-[10px]" textAnchor={isRtl ? 'start' : 'end'}>{prices.length} {t('general.periods')}</text>
        {prices.map((_, i) => {
          const pt = getPoint(i);
      ...
      {active && activePt && (
        ...
        >
          ...
          <div className="space-y-0.5">
            <div><span className="text-gray-500 dark:text-gray-400">{t('propertyDetail.chartPeriod')}</span> {active.period}</div>
            <div><span className="text-gray-500 dark:text-gray-400">{t('propertyDetail.chartPrice')}</span> <span className="font-semibold text-emerald-600 dark:text-emerald-400">{fmt(active.price)}</span></div>
            {diff !== null && (
              <>
                <div className={`${diff >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                  <span className="text-gray-500 dark:text-gray-400">{t('propertyDetail.chartChange')}</span> {fmtDiff(diff)} ({fmtPct(diffPct)})
                </div>
                <div className="text-gray-400 dark:text-gray-500">
                  <span className="text-gray-500 dark:text-gray-400">{t('propertyDetail.chartPrevious')}</span> {fmt(prevPrice)}
                </div>
              </>
            )}
          </div>
          ...
          {pinned && (
            <button
              onClick={() => { setPinned(false); setActiveIdx(null); }}
...
export default function PropertyPage() {
  ...
  if (error || !data) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <p className="text-red-600 dark:text-red-400">{error ? translateError(new Error(error), t) : t('errors.Property not found')}</p>
      </div>
    );
  }
    ...
    <div className="flex-1 p-4 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                ...
                <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded">
                  ...
                  <p className="text-lg font-semibold text-red-600 dark:text-red-400">${property.maintenanceCost?.toLocaleString()}</p>
                </div>
              </div>
              <div className="max-h-64 overflow-y-auto space-y-1">
                {property.units.slice(0, 50).map((unit, i) => (
                  <div key={i} className="bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded text-sm flex justify-between items-center">
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-emerald-600 dark:text-emerald-400">${unit.rentPrice?.toLocaleString()}/period</span>
                      <span className={`text-xs px-2 py-0.5 rounded ${unit.occupied ? 'bg-emerald-900 text-emerald-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
                ...
                {property.units.length > 50 && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 text-center mt-2">+{property.units.length - 50} more units</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          {actionMsg && (
            <div className={`p-3 rounded text-sm ${actionMsg.type === 'success' ? 'bg-emerald-900 text-emerald-300' : 'bg-red-900 text-red-300'}`}>
              {actionMsg.text}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              {t('propertyDetail.marketValue')}: ${property.currentPrice?.toLocaleString()}<br />
