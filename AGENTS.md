...
- **Deep-link metadata**: notification clicks navigate via server-set `route` (+ structured `tab`/`subTab`/`proposalId`/`auctionId`/`contractId` fields merged into the URL by `NotificationsPage`). Company contract notifications (eventKeys `company:{id}:contract:{contractId}:available|vote_request|submitted|started|completed|proposal_approved|proposal_expired|expired`) set `route: /real-estate-companies/{companyId}`, `tab: contracts`, `subTab` (available/proposed/active/history) and `contractId` so clicking lands the player on the right contracts sub-view with the contract highlighted (`#company-contract-{id}` ring in `CompanyDetailPage`). Authorization stays server-authoritative — the fields are navigation metadata only.
